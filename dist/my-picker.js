...
function Wheel(picker, col, option, index) {
	...
	this.index = index;
	//转轮主体
	this.dom = $('<div class="picker-wheel">' + '<div class="picker-label"><span class="picker-text"></span></div>' + '<ul></ul>' + '<div class="picker-label"><span class="picker-text"></span></div>' + '</div>').css('height', config.wheelHeight + 'vmin');
...
Wheel.prototype.setOptions = function (list, selectedValue, isInti) {
	...
	this.list.forEach(function (item, index) {
		...
		li.css("transform", "rotateX(" + angle + "deg) translateZ(" + that.radius + "vmin) scale(0.75)").css("padding", height / 5.9 + "vmin 0").css("height", height + "vmin").css("line-height", height + "vmin");
		//将标签的角度保存到其dom中
		li.data("angle", angle);
		//将标签的index保存到其dom中
		li.data("index", i);
